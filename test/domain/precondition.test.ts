...
describe('Precondition', function() {
  describe('#evaluate()', function() {
    it('Platforms match', function() {
      ...
      const actual = precondition.evaluate({ injected_platform_type: Platform.LINUX.toString() });
      assert.equal(actual, true);
    });
    it('Platforms mismatch', function() {
      const precondition = Precondition.newPlatformPrecondition(Platform.LINUX);
      const actual = precondition.evaluate({ injected_platform_type: Platform.WINDOWS.toString() });
